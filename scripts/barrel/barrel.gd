...
func _explode_others():
	for bodie in $explosion.get_overlapping_bodies():
		if bodie.has_method("take_damage") and bodie != self:
			if "durability" in bodie:
				if bodie.durability > 0:
					var explosion_distance = (
						5
						* bodie.global_transform.origin.distance_to(global_transform.origin)
					)
					bodie.take_damage(300 - explosion_distance)
