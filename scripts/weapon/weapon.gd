class Weapon:
	...
	func sprint(sprint, _delta) -> void:
			...
			if animc != "Shoot" and animc != "Reload" and animc != "Draw" and animc != "Hide":
				...
				if ray.is_colliding():
					...
					if ray.get_collider() is RigidBody:
						ray.get_collider().apply_central_impulse(
							-ray.get_collision_normal() * (local_damage * 0.3)
						)
					...
					if ray.get_collider().is_in_group("prop"):
						...
						if ray.get_collider().has_method("take_damage"):
							ray.get_collider().take_damage(local_damage)
					...
					decal.set_scale(Vector3.ONE * 0.005)
					...
					decal.look_at(
						ray.get_collision_point() + ray.get_collision_normal(), Vector3(1, 1, 0)
					)
		else:
			# Play out sound
	...
	func update(_delta) -> void:
		...
		# Change light energy
		effect.get_node("shoot").light_energy = lerp(
			effect.get_node("shoot").light_energy, 0, 5 * _delta
		)
